        ...
        {projectITems.map((project, index) => (
          <motion.article initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: 1 * 0.2 }} key={'project_'+index} className="p-6 border dark:border-gray-800 border-[#f1f1f1] rounded-lg bg-[#f1f1f1] dark:bg-[#06060b] hover:ring-black/30 transition">
            ...
            <span className="flex gap-2">
              {(project.webUrl && project.webUrl.trim() !== '') && <a href={project.webUrl} target="_blank" className="text-gray-400 dark:text-gray-600 hover:text-gray-600 dark:hover:text-white/90 inline-block mt-2"><BiGlobe className="text-xl md:text-2xl" /><span className="sr-only">Web Application</span></a>}
              {(project.mobileUrl && project.mobileUrl.trim() !== '') && <a href={project.mobileUrl} target="_blank" className="text-gray-400 dark:text-gray-600 hover:text-gray-600 dark:hover:text-white/90 inline-block mt-2"><BiLogoPlayStore className="text-xl md:text-2xl" /><span className="sr-only">Get Mobile App</span></a>}
            </span>
            </p>
